...
// ----------------- JSON Update / Merge -----------------
export async function update(key: string, path: string, value: any) 
{
  // Get current object at path (await the call so we don't handle a Promise later)
  const current = await rediss.call(
    "JSON.GET",
    key,
    path.startsWith("$") ? path : `$.${path}`
  );
  // Coerce to string before parsing to avoid casting Promise<unknown> -> string
